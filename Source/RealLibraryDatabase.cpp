...
bool RealLibraryDatabase::deleteBook(QString id){
    int pos=getBookPos(id);
    if(this->getBorrowed(id)==0){
        this->List.erase(this->List.begin()+pos);
        return true;
    }
    else{
        this->List[pos].updateQuantiy(0);
        return false;
    }
}
...
void RealLibraryDatabase::deleteBook(QString id, int num){
    int pos=getBookPos(id);
    if(num<List[pos].getNumOfRemaingBooks())
        this->List[pos].updateQuantiy(List[pos].getNumOfRemaingBooks()-num);
    else this->List[pos].updateQuantiy(0);
}
...
int RealLibraryDatabase::getBorrowed(QString ID)
{
    for(auto book:List){
        if(book.getID()==ID)
            return book.getNumOfOfBorrowedBooks();
    }
    return -1;
}

  bool RealLibraryDatabase::updateBorrowed(QString ID,int num)
  {
      int size=List.size();
      for(int i=0;i<size;i++){
          if(QString::compare(List[i].getID(),ID)==0){
              List[i].updateBorrowedBooks(num);
              return true;
          }
      }
      return false;
